...
const svgs = {
    ...
    Instagram: `<svg viewBox="0 0 24 24" width="45" height="45" xmlns="http://www.w3.org/2000/svg"><defs><linearGradient id="ig-grad-lg" x1="1" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#405de6" /><stop offset="25%" stop-color="#5851db" /><stop offset="50%" stop-color="#833ab4" /><stop offset="75%" stop-color="#c13584" /><stop offset="100%" stop-color="#e1306c" /></linearGradient></defs><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm3.98-10.98a1.44 1.44 0 110 2.881 1.44 1.44 0 010-2.88z" fill="url(#ig-grad-lg)"/></svg>`
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.showCustomToast = function(toastMessageText, isErrorAlert = false) {
        ...
        if (preExistingToast) { preExistingToast.remove(); }
        ...
        setTimeout(() => { if (toastDomElement) toastDomElement.remove(); }, 3500);
    };
    ...
    if (buttonOpenFeatures && overlayFeaturesModal) {
        buttonOpenFeatures.addEventListener('click', () => { overlayFeaturesModal.classList.add('active'); });
    }
    if (buttonCloseFeatures && overlayFeaturesModal) {
        buttonCloseFeatures.addEventListener('click', () => { overlayFeaturesModal.classList.remove('active'); });
    }
    ...
    if (authLoginForm) {
        authLoginForm.addEventListener('submit', function(submitEvent) {
            ...
            if (providedEmailValue && providedPasswordValue.length >= 6) {
                ...
                if(extractedUserName.length > 0) { extractedUserName = extractedUserName.charAt(0).toUpperCase() + extractedUserName.slice(1); } 
                else { extractedUserName = "Student"; }
    ...
    // ==========================================
    // 6. 🚀 NEW: REAL OAUTH CONSENT LOGIC (Google/FB/IG)
    // ==========================================
    ...
    // Step 1: Open the Consent Popup
    window.openOAuthConsent = function(providerName) {
        triggerClickSound();
        
        // Setup the modal UI based on provider
        oauthProviderNameText.innerText = providerName;
        oauthProviderIconContainer.innerHTML = svgs[providerName];
        
        // Change Allow Button Color matching the brand
        if(providerName === 'Google') oauthAllowBtn.style.background = '#4285F4';
        if(providerName === 'Facebook') oauthAllowBtn.style.background = '#1877F2';
        if(providerName === 'Instagram') oauthAllowBtn.style.background = '#e1306c';
        
        // Store which provider requested auth
        oauthModal.setAttribute('data-requesting-provider', providerName);
    ...
    if (oauthAllowBtn) {
        oauthAllowBtn.addEventListener('click', () => {
            triggerClickSound();
            const activeProvider = oauthModal.getAttribute('data-requesting-provider');
    ...
    if (backgroundCanvasNode) {
        ...
        class LoginFireflyParticle {
            constructor() {
                this.particleShape = Math.random() > 0.4 ? 'dot' : 'symbol';
            ...
            updatePositionData() {
                ...
                if (this.coordinateY < -30) { this.coordinateY = backgroundCanvasNode.height + 30; this.coordinateX = Math.random() * backgroundCanvasNode.width; }
                if (this.coordinateX < -30 || this.coordinateX > backgroundCanvasNode.width + 30) { this.velocityVectorX = this.velocityVectorX * -1; }
            }
        ...
        for (let iterationIndex = 0; iterationIndex < 40; iterationIndex++) { activeParticlesCollection.push(new LoginFireflyParticle()); }
        function executeBackgroundAnimation() { renderContext2D.clearRect(0, 0, backgroundCanvasNode.width, backgroundCanvasNode.height); activeParticlesCollection.forEach(particleItem => { particleItem.updatePositionData(); particleItem.drawOntoCanvas(renderContext2D); }); requestAnimationFrame(executeBackgroundAnimation); }
        executeBackgroundAnimation();
        window.addEventListener('resize', () => { backgroundCanvasNode.width = window.innerWidth; backgroundCanvasNode.height = window.innerHeight; });
    }
